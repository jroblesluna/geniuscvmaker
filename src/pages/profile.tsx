...
function Profile({ auth }) {
            ...
            switch (field) {
                case "firstName":
                    if (!/^[^0-9±!@£$%^&*_+§¡€#¢§¶•ªº«\<>-?:;|=.,]{2,50}$/.test(value)) {
                        isValid = false;
                        toast.error('First name must contain only alphabetic characters and spaces, and be between 2 and 50 characters long');
                    }
                    break;
                case "lastName":
                    if (!/^[^0-9±!@£$%^&*_+§¡€#¢§¶•ªº«\<>-?:;|=.,]{2,50}$/.test(value)) {
                        isValid = false;
                        toast.error('Last name must contain only alphabetic characters and spaces, and be between 2 and 50 characters long');
                    }
                    break;
                case "email":
                    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) {
            ...
            if (isValid) {
                ...
                if (field === "addressState") {
                    await updateDoc(userDocRef, {
                        ...
                        ["addressCountryCode"]: CountryRegionData.find(data => data[0] === userData?.["addressCountry"])?.[1],
                ...
                setOriginalUserData(prevUserData => ({ ...prevUserData, [field]: userData?.[field] }));
            }
        } catch (error) {
    ...
    const handleCancelEdit = (field: string) => {
        if (field === "addressState") {
            setUserData((prevUserData) => ({ ...prevUserData, ["addressCountry"]: originalUserData?.["addressCountry"] }));
        }
                                ...
                                {editableFields["addressState"] ? (
                                    <div className="mb-4">
                                        <div className="flex items-center">
                                            ...
                                            <button onClick={() => handleSaveField("addressState")} className="text-green-500"><SvgCheck /></button>
                                            <button onClick={() => handleCancelEdit("addressState")} className="text-red-500"><SvgCancel /></button>
                                        </div>
                                    </div>) : (
                                    <div className="flex items-center">
                                        <div className='grid grid-cols-1 gap-2 w-full mt-1 mr-2'>
                                        ...
                                        <button onClick={() => handleEditField("addressState")} className="topic-title-blue"><SvgEdit /></button>
                                    </div>
                                )}
                            </div>
