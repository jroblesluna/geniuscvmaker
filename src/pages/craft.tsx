...
function Craft({ auth }) {
  ...
  const fetchCvCrafts = async () => {
    ...
    const cvCraftsRef = collection(firestore, 'users/' + user.uid + '/cvCrafts');
    const cvCraftsSnapshot = await getDocs(cvCraftsRef);

    const cvCraftsList: CvCraft[] = [];
    cvCraftsSnapshot.forEach((doc) => {
      const cvRequestData = doc.data() as CvCraft;
      cvCraftsList.push({
        id: doc.id,
        firstName: cvRequestData.firstName,
        lastName: cvRequestData.lastName,
        email: cvRequestData.email,
        telephoneNumber: cvRequestData.telephoneNumber,
        about: cvRequestData.about,
        facebook: cvRequestData.facebook,
        instagram: cvRequestData.instagram,
        linkedin: cvRequestData.linkedin,
        tiktok: cvRequestData.tiktok,
        youtube: cvRequestData.youtube,
        github: cvRequestData.github,
        workExperiences: cvRequestData.workExperiences,
        skills: cvRequestData.skills,
        languages: cvRequestData.languages,
        licenses: cvRequestData.licenses,
        certifications: cvRequestData.certifications,
        personalReferences: cvRequestData.personalReferences,
      });
    });

    console.log(cvCraftsList);
    ...
    try {
      if (isEditing && currentCv?.id) {
        ...
        const newCurrentCv = {
          ...currentCv,
          id: currentCv?.id || '',
          firstName: currentCv?.firstName || '',
          lastName: currentCv?.lastName || '',
          email: currentCv?.email || '',
          telephoneNumber: currentCv?.telephoneNumber || '',
          about: currentCv?.about || '',
          facebook: currentCv?.facebook || '',
          instagram: currentCv?.instagram || '',
          linkedin: currentCv?.linkedin || '',
          tiktok: currentCv?.tiktok || '',
          youtube: currentCv?.youtube || '',
          github: currentCv?.github || '',
          workExperiences: currentCv?.workExperiences || [],
          licenses: currentCv?.licenses || [],
          certifications: currentCv?.certifications || [],
          skills: currentCv?.skills || [],
          languages: currentCv?.languages || [],
          personalReferences: currentCv?.personalReferences || [],
        };

        await addDoc(cvCraftsRef, newCurrentCv);
    ...
    setCurrentCv((currentCv) =>
      currentCv
        ? {
            ...
            licenses: [
              ...currentCv.licenses,
              { institution: '', achieved: '', degree: '', title: '' },
            ],
          }
        : null
    );
    ...
    setCurrentCv((currentCv) =>
      currentCv
        ? {
            ...currentCv,
            personalReferences: [
              ...currentCv.personalReferences,
              { name: '', institution: '', position: '', telephoneNumber: '', email: '' },
            ],
          }
        : null
    );
        ...
        <>
          ...
          {cvCrafts.length != 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {cvCrafts.map((cv) => (
                <Card key={cv.id} className="mb-4 p-2">
                  ...
                  <CardBody>
                    ...
                    <p className="text-[14px]">
                      <b>Phone:</b> {cv.telephoneNumber}
                    </p>
                    <p className="text-[14px] line-clamp-2">
                      <b>Description:</b> {cv.about}
                    </p>
                    <div className="grid grid-cols-2 gap-2 mt-2">
                      <Button
                        variant="ghost"
                        isDisabled={isUpdatingState[cv.id ?? ''] || false}
                        onClick={() => handleWorkExperiencesModalOpen(cv)}
                      >
                        Work Experiences ({cv.workExperiences.length})
                      </Button>
                      <Button
                        variant="ghost"
                        isDisabled={isUpdatingState[cv.id ?? ''] || false}
                        onClick={() => handleLicensesModalOpen(cv)}
                      >
                        Licenses ({cv.licenses.length})
                      </Button>
                      <Button
                        variant="ghost"
                        isDisabled={isUpdatingState[cv.id ?? ''] || false}
                        onClick={() => handleCertificationsModalOpen(cv)}
                      >
                      ...
                      <Button
                        variant="ghost"
                        isDisabled={isUpdatingState[cv.id ?? ''] || false}
                        onClick={() => handleSkillsModalOpen(cv)}
                      >
                      ...
                      <Button
                        variant="ghost"
                        isDisabled={isUpdatingState[cv.id ?? ''] || false}
                        onClick={() => handleLanguagesModalOpen(cv)}
                      >
                      ...
                      <Button
                        variant="ghost"
                        isDisabled={isUpdatingState[cv.id ?? ''] || false}
                        onClick={() => handlePersonalReferencesModalOpen(cv)}
                      >
                        References ({cv.personalReferences.length})
                      </Button>
                    </div>
                  </CardBody>
                  <CardFooter className="flex flex-row items-center gap-4">
                    <div className="text-xl font-bold text-left w-full flex flex-row gap-2">
                      {socialLinks.map(({ name, baseUrl }) => (
                        <a
                          key={name}
                          href={cv[name].length > 0 ? `${cv[name]}` : undefined}
                          target={cv[name].length > 0 ? '_blank' : undefined}
                    ...
                    <Button
                      ...
                      className="appBlackOnCitrine w-full"
                    >
                      Craft CV
                    </Button>
                  </CardFooter>
            ...
            <div className="text-center flex flex-col items-center h-[50vh] justify-center  mx-auto  ">
              ...
              <p className="mt-2">There are no Crafted CVs yet!</p>
            </div>
          )}
        </>
      )}
          ...
          <ModalContent>
            <ModalHeader className="flex flex-row items-center">
              ...
              <span className="text-sm font-light text-right w-full">
                Complete with most accurate information
              </span>
            </ModalHeader>
            <ModalBody>
              <div className="flex flex-col">
                <div className="mb-4">
                  Please review the following information, it's going to be used for generate
                  your CV.
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {socialLinks.map((socialNetwork) => (
                    <Input
                      ...
                      label={
                        socialNetwork.name.charAt(0).toUpperCase() +
                        socialNetwork.name.slice(1)
                      }
                      key={socialNetwork.name}
                      value={currentCv[socialNetwork.name]}
                      onChange={handleInputChange}
                      fullWidth
                      placeholder={socialNetwork.baseUrl}
                      startContent={
                        <>
                          <img
                            src={`/assets/svg/${socialNetwork.name}.svg`}
                            width={20}
                            className="mr-1"
                          />
                        </>
            ...
            <ModalFooter>
              <Button onClick={handleCreateUpdateCV} className="bg-lime-500">
            ...
            {(onWorkExperiencesModalClose) => (
              <>
                ...
                <ModalBody>
                  <div className="flex flex-col gap-4">
                    {currentCv.workExperiences.map((workExperience, index) => (
                      <div
                        key={index}
                        className=" bg-gray-50 border border-gray-300 p-2 rounded-lg shadow-lg"
                      >
                        ...
                        <div className="flex flex-col sm:flex-row gap-2">
                          <I18nProvider locale="en-CA">
                            <DatePicker
                              ...
                              label="Date From"
                              value={
                                workExperience.dateFrom.length > 0
                                  ? parseDate(workExperience.dateFrom)
                                  : undefined
                              }
                              onChange={(event) =>
                                handleWorkExperienceChange(event, index, 'dateFrom')
                              }
                              fullWidth
                              variant="underlined"
                            />
                            <DatePicker
                              ...
                              label="Date To"
                              value={
                                workExperience.dateTo.length > 0
                                  ? parseDate(workExperience.dateTo)
                                  : undefined
                              }
                              onChange={(event) =>
                                handleWorkExperienceChange(event, index, 'dateTo')
                              }
                              fullWidth
                ...
                <ModalFooter>
                  ...
                  <Button color="danger" onClick={onWorkExperiencesModalClose}>
                    Cancel
                  </Button>
                  <Button
                    color="success"
                    onClick={() => handleWorkExperiencesModalClose(true)}
                  >
                    Save and Close
                  </Button>
                </ModalFooter>
          ...
          <ModalContent>
            {(onLicensesModalClose) => (
              <>
                        ...
                        <div className="flex flex-row gap-2">
                          <I18nProvider locale="en-CA">
                            <DatePicker
                              ...
                              label="Achieved"
                              value={
                                license.achieved.length > 0
                                  ? parseDate(license.achieved)
                                  : undefined
                              }
                              onChange={(event) =>
                                handleLicenseChange(event, index, 'achieved')
                              }
                              fullWidth
                              variant="underlined"
                            />
                ...
                <ModalFooter>
                  <Button color="primary" onClick={handleAddLicense}>
                    Add License
                  </Button>
                  <Button color="danger" onClick={onLicensesModalClose}>
                    Cancel
                  </Button>
                  <Button color="success" onClick={() => handleLicensesModalClose(true)}>
                    Save and Close
                  </Button>
                </ModalFooter>
              </>
            ...
            {(onCertificationsModalClose) => (
              <>
                        ...
                        <div className="flex flex-row gap-2">
                          <I18nProvider locale="en-CA">
                            <DatePicker
                              ...
                              label="Achieved"
                              value={
                                certification.achieved.length > 0
                                  ? parseDate(certification.achieved)
                                  : undefined
                              }
                              onChange={(event) =>
                                handleCertificationChange(event, index, 'achieved')
                              }
                              fullWidth
                              variant="underlined"
                            />
                          ...
                          <I18nProvider locale="en-CA">
                            <DatePicker
                              ...
                              label="Expiration"
                              value={
                                certification.expiration.length > 0
                                  ? parseDate(certification.expiration)
                                  : undefined
                              }
                              onChange={(event) =>
                                handleCertificationChange(event, index, 'expiration')
                              }
                              fullWidth
                ...
                <ModalFooter>
                  ...
                  <Button color="danger" onClick={onCertificationsModalClose}>
                    Cancel
                  </Button>
                  <Button color="success" onClick={() => handleCertificationsModalClose(true)}>
                    Save and Close
                  </Button>
                </ModalFooter>
              </>
            ...
            {(onSkillsModalClose) => (
              <>
                <ModalHeader className="flex flex-row text-md">
                  ...
                  <span className="text-sm font-light text-right w-full">
                    Example: Python, Management, Teamwork...
                  </span>
                </ModalHeader>
                <ModalBody>
                  <div className="flex flex-col gap-4">
                ...
                <ModalFooter>
                  ...
                  <Button color="danger" onClick={onSkillsModalClose}>
                    Cancel
                  </Button>
                  <Button color="success" onClick={() => handleSkillsModalClose(true)}>
                    Save and Close
                  </Button>
                </ModalFooter>
              </>
            ...
            {(onLanguagesModalClose) => (
              <>
                    ...
                    {currentCv.languages.map((language, index) => (
                      ...
                      >
                        <div className="flex flex-row gap-2">
                          ...
                          >
                            {(proficiencyLevel) => (
                              <SelectItem key={proficiencyLevel.key}>
                ...
                <ModalFooter>
                  ...
                  <Button color="danger" onClick={onLanguagesModalClose}>
                    Cancel
                  </Button>
                  <Button color="success" onClick={() => handleLanguagesModalClose(true)}>
                    Save and Close
                  </Button>
                </ModalFooter>
              </>
        ...
        >
          <ModalContent>
            {(onPersonalReferencesModalClose) => (
              <>
                <ModalHeader className="flex flex-row items-center">
                  <span className="text-xl font-bold text-left w-fit">
                    Personal References
                  </span>
                  <span className="text-sm font-light text-right w-full">
                    Complete with most accurate information
                ...
                <ModalFooter>
                  <Button color="primary" onClick={handleAddPersonalReference}>
                    Add Personal Reference
                  </Button>
                  <Button color="danger" onClick={onPersonalReferencesModalClose}>
                    Cancel
                  </Button>
                  <Button
                    color="success"
                    onClick={() => handlePersonalReferencesModalClose(true)}
                  >
                    Save and Close
                  </Button>
                </ModalFooter>
