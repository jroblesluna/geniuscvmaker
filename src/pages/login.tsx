...
import { goHome } from '../utils/navigateRoutes';
...
function Login({ auth }) {
  ...
  const createStripeCostumer = async (email: string, name: string) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Error al crear el Setup Intent');
      }

      console.log('Setup Intent:', data.setupIntent);
  ...
  async function createCulqiUser(
    email: string,
    address: string,
    address_city: string,
    country_code: string,
          ...
          if (additionalUserInfo !== null) {
            ...
            if (!userSnapshot.exists() && profile) {
              try {
                ...
                // let culqiCustomer = await createCulqiUser(
                //   userFromUserCred.email as string,
                //   newAddress,
                //   newAddressCity,
                //   newAddressCountryCode,
                //   profile.given_name as string,
                //   profile.family_name as string,
                //   newPhoneNumber
                // );

                let costumerId = await createStripeCostumer(
                  userFromUserCred.email || '',
                ...
                //   try {
                //     culqiCustomer = (
                //       await listCulqiCustomerByEmail(userFromUserCred.email as string)
                //     ).data[0];
                //   } catch (error) {
                //     toast.error('Error consultando cliente existente.');
                //   }

                console.log('Customer ID', costumerId);
                ...
                setIsNewUser(true);
                toast.success(
                  'A sample professional review was AI created for you! Update your profile!'
                );
              } catch (error) {
    ...
    <>
          ...
          <div className="m-2 grid justify-center items-center">
            ...
            <div className="m-2 flex justify-center items-center">
              <Button
                ...
                isLoading={isWorking}
                onClick={onOpen}
                className="p-4 w-fit rounded-xl text-lg bg-gray-200"
              >
                Sign in with Google
              </Button>
            </div>
            <div
              onClick={goHome}
              className="m-2 flex justify-center items-center topic-text-white cursor-pointer"
            >
              ← Go back to Home
            </div>
            <div>
      ...
      >
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1">{tosTitle}</ModalHeader>
              <ModalBody className="bg-gray-200 mx-3">{tosContent}</ModalBody>
              <ModalFooter>
