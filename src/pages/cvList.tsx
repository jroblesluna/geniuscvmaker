  ...
  const handlePrint = () => {
    ...
    if (printWindow) {
      fetch('/assets/html/printTemplate.html')
        ...
        .then(data => {
          const modifiedHTML = data.replace('<div id="content"></div>', `<div id="content">${selectedCV || ''}</div>`);
      ...
      <div className={`flex ${isSmallScreen ? "flex-col" : "flex-row"} gap-4`}>
        <div className={`${isSmallScreen ? "w-full" : "w-1/2"}`}>
          {isLoading ? (
            ...
            cvRequests.length === 0 ? (
              <div className='text-center'>There are no CV Requests yet!</div>
            ) : (
              <div className='flex flex-col w-full gap-4'>
                {cvRequests
                  .sort((a, b) => b.createdAt.toMillis() - a.createdAt.toMillis())
                  .map((cvRequest) => (
                    <div key={cvRequest.id} className='bg-gradient-to-tr from-blue-950 to-blue-300 rounded-md shadow-blue-950 shadow-md flex flex-row border gap-2'>
                      <div>
                      ...
                      <div className='flex flex-col w-full justify-center text-[#DDEBFF]'>
                        <div><b>Date:</b> {cvRequest.createdAt.toDate().toLocaleString()}</div>
        ...
        <div className={`${isSmallScreen ? "w-full" : "w-1/2"}`}>
          {showCV && selectedCV && (
            <div className={`${isSmallScreen ? "" : "sticky top-0 max-h-screen overflow-auto"}`}>
              {isSmallScreen ? (
                <div className="fixed inset-0 flex items-center justify-center z-50 p-2">
                  <div className="bg-black bg-opacity-50 absolute inset-0" onClick={handleHideCV}></div>
                  <div className="bg-white p-6 rounded-lg shadow-lg z-10 max-w-full max-h-full overflow-auto">
                    <div className="flex justify-between items-center mb-4">
                      <button onClick={handlePrint} className="text-blue-500 hover:text-blue-700">Print</button>
                      <button onClick={handleDownloadWord} className="text-blue-500 hover:text-blue-700">Download as Word</button>
                      <button onClick={handleHideCV} className="text-red-500 hover:text-red-700">Close</button>
                    </div>
