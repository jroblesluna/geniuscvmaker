...
const callPdf = async (pdfUrl) => {
  try {
    const response = await fetch(`http://localhost:3000/api/extract-pdf?url=${pdfUrl}`);
      ...
      switch (pendingTaskData.geniusApp) {
        case 'scratch':
          ...
          scratchInterview +=
            ...
            '\n';
          scratchInterview +=
            'Question: ' + scratchQuestions.skills + '\nAnswer: ' + cvInputData.skills + '\n';
          scratchInterview +=
            'Question: ' +
        ...
        case 'craft':
          ...
          cvInputData.certifications.map((certification, index) => {
            ...
            certification.certificationID &&
              (craftData += 'My Certification ID is ' + certification.certificationID + '\n');
            certification.certificationURL &&
              (craftData += 'The Certificate URL is ' + certification.certificationURL + '\n');
          });
